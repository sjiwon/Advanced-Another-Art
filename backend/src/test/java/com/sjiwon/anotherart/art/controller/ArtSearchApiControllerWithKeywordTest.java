...
class ArtSearchApiControllerWithKeywordTest extends ControllerTest {
    ...
    class searchArt {
        ...
        void test2() throws Exception {
            ...
            List<AuctionArt> filteringAuctionArtWithKeyword = filteringAuctionArtWithKeyword(auctionArtList, KEYWORD_HELLO);
            given(auctionArtComplexSearchService.getAuctionArtListByKeyword(KEYWORD_HELLO, sort, DEFAULT_PAGE_REQUEST))
                    .willReturn(PageableExecutionUtils.getPage(filteringAuctionArtWithKeyword, DEFAULT_PAGE_REQUEST, () -> 6L));
            ...
            mockMvc.perform(requestBuilder)
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.contentSize").exists())
                    .andExpect(jsonPath("$.contentSize").value(filteringAuctionArtWithKeyword.size()))
                    .andExpect(jsonPath("$.artList").exists())
                    .andExpect(jsonPath("$.pagination").exists())
        ...
        void test3() throws Exception {
            // given
            final String sort = "date";
            List<GeneralArt> generalArtList = createGeneralArtList(TOTAL_ELEMENTS, COMMON_NAME_AND_DESCRIPTION, COMMON_DEFAULT_HASHTAGS);
            sortGeneralArtByDate(generalArtList);

            List<GeneralArt> filteringGeneralArtWithKeyword = filteringGeneralArtWithKeyword(generalArtList, KEYWORD_HELLO);
            given(generalArtComplexSearchService.getGeneralArtListByKeyword(KEYWORD_HELLO, sort, DEFAULT_PAGE_REQUEST))
                    .willReturn(PageableExecutionUtils.getPage(filteringGeneralArtWithKeyword, DEFAULT_PAGE_REQUEST, () -> 6L));
            ...
            mockMvc.perform(requestBuilder)
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.contentSize").exists())
                    .andExpect(jsonPath("$.contentSize").value(filteringGeneralArtWithKeyword.size()))
                    .andExpect(jsonPath("$.artList").exists())
                    .andExpect(jsonPath("$.pagination").exists())
                    .andExpect(jsonPath("$.pagination.totalElements").value(6))
                    .andExpect(jsonPath("$.pagination.totalPages").value(1))
