...
@Import(ArtBasicJooqRepository.class)
@DisplayName("Art -> ArtBasicQueryRepository 테스트")
class ArtBasicQueryRepositoryTest extends RepositoryTest {
    @Autowired
    private ArtBasicJooqRepository sut;
    ...
    class GetArtType {
        ...
        void success() {
            assertAll(
                    () -> assertThat(sut.getArtType(artA.getId())).isEqualTo(GENERAL.name()),
                    () -> assertThat(sut.getArtType(artB.getId())).isEqualTo(AUCTION.name())
            );
        }
    }
