...
@DisplayName("Point [Repository Layer] -> PointDetailRepository 테스트")
class PointDetailRepositoryTest extends RepositoryTest {
    ...
    void test1() {
        ...
        assertAll(
                () -> assertThat(pointDetails2.size()).isEqualTo(1),
                // 포인트 충전
                () -> assertThat(pointDetails2.get(0).getPointType()).isEqualTo(PointType.CHARGE),
                () -> assertThat(pointDetails2.get(0).getAmount()).isEqualTo(CHARGE_AMOUNT),
        ...
        assertAll(
                () -> assertThat(pointDetails3.size()).isEqualTo(1),
                // 작품 판매
                () -> assertThat(pointDetails3.get(0).getPointType()).isEqualTo(PointType.SOLD),
                () -> assertThat(pointDetails3.get(0).getAmount()).isEqualTo(art.getPrice()),
        ...
        assertAll(
                () -> assertThat(pointDetails4.size()).isEqualTo(2),
                // 포인트 충전
                () -> assertThat(pointDetails4.get(0).getPointType()).isEqualTo(PointType.CHARGE),
                () -> assertThat(pointDetails4.get(0).getAmount()).isEqualTo(CHARGE_AMOUNT),
                () -> assertThat(pointDetails4.get(0).getMember().getId()).isEqualTo(member.getId()),
                // 작품 구매
                () -> assertThat(pointDetails4.get(1).getPointType()).isEqualTo(PointType.PURCHASE),
                () -> assertThat(pointDetails4.get(1).getAmount()).isEqualTo(art.getPrice()),
        ...
        assertAll(
                () -> assertThat(pointDetails5.size()).isEqualTo(3),
                // 포인트 충전
                () -> assertThat(pointDetails5.get(0).getPointType()).isEqualTo(PointType.CHARGE),
                () -> assertThat(pointDetails5.get(0).getAmount()).isEqualTo(CHARGE_AMOUNT),
                () -> assertThat(pointDetails5.get(0).getMember().getId()).isEqualTo(member.getId()),
                // 작품 구매
                () -> assertThat(pointDetails5.get(1).getPointType()).isEqualTo(PointType.PURCHASE),
                () -> assertThat(pointDetails5.get(1).getAmount()).isEqualTo(art.getPrice()),
                () -> assertThat(pointDetails5.get(1).getMember().getId()).isEqualTo(member.getId()),
                // 포인트 환불
                () -> assertThat(pointDetails5.get(2).getPointType()).isEqualTo(PointType.REFUND),
                () -> assertThat(pointDetails5.get(2).getAmount()).isEqualTo(REFUND_AMOUNT),
    ...
    private void doCharge(Member member, int amount) {
        pointDetailRepository.save(PointDetail.insertPointDetail(member, PointType.CHARGE, amount));
    }

    private void doPurchase(Art art, Member owner, Member buyer) {
        art.changeArtStatus(ArtStatus.SOLD_OUT);
        // 판매자 포인트
        owner.increasePoint(art.getPrice());
        pointDetailRepository.save(PointDetail.insertPointDetail(owner, PointType.SOLD, art.getPrice()));
        // 구매자 포인트
    ...
    private void doRefund(Member member, int amount) {
        pointDetailRepository.save(PointDetail.insertPointDetail(member, PointType.REFUND, amount));
    }
