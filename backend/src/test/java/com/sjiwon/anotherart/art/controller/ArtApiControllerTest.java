...
class ArtApiControllerTest extends ControllerTest {
    ...
    class registrationArt {
        ...
        void test3() throws Exception {
            ...
            final String message = "작품 이미지가 업로드되지 않았습니다.";
            mockMvc.perform(requestBuilder)
        ...
        void test4() throws Exception {
            ...
            // then
            final GlobalErrorCode expectedError = GlobalErrorCode.VALIDATION_ERROR;
            final String message = "이미지는 jpg, jpeg, png만 허용합니다.";
            mockMvc.perform(requestBuilder)
                    .andExpect(status().isBadRequest())
                    .andExpect(jsonPath("$.statusCode").exists())
                    .andExpect(jsonPath("$.statusCode").value(expectedError.getStatus().value()))
        ...
        void test5() throws Exception {
            ...
            // then
            final GlobalErrorCode expectedError = GlobalErrorCode.VALIDATION_ERROR;
            final String message = String.format("작품은 최소 %d개의 해시태그를 가져야 합니다.", 1);
            mockMvc.perform(requestBuilder)
                    .andExpect(status().isBadRequest())
                    .andExpect(jsonPath("$.statusCode").exists())
                    .andExpect(jsonPath("$.statusCode").value(expectedError.getStatus().value()))
        ...
        void test6() throws Exception {
            ...
            // then
            final GlobalErrorCode expectedError = GlobalErrorCode.VALIDATION_ERROR;
            final String message = String.format("작품은 최대 %d개의 해시태그를 가질 수 있습니다.", 10);
            mockMvc.perform(requestBuilder)
                    .andExpect(status().isBadRequest())
                    .andExpect(jsonPath("$.statusCode").exists())
                    .andExpect(jsonPath("$.statusCode").value(expectedError.getStatus().value()))
        ...
        void test8() throws Exception {
            ...
            // then
            final GlobalErrorCode expectedError = GlobalErrorCode.VALIDATION_ERROR;
            final String message = "경매 시작 날짜는 현재 시간 이후여야 합니다.";
            mockMvc.perform(requestBuilder)
                    .andExpect(status().isBadRequest())
                    .andExpect(jsonPath("$.statusCode").exists())
                    .andExpect(jsonPath("$.statusCode").value(expectedError.getStatus().value()))
