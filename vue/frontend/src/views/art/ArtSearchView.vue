<template>
  <div class="container">
    ...
    <div class="clearfix text-center">
      <b-dropdown v-if="$store.getters.isCurrentRequestEqualsAuctionArt === true" size="lg" v-model:text="sortType.defaultSort" split variant="outline-secondary">
      ...
      <b-dropdown v-if="$store.getters.isCurrentRequestEqualsGeneralArt === true" size="lg" v-model:text="sortType.defaultSort" split variant="outline-secondary">
      ...
      <div class="container">
        <div class="row row-cols-sm-2 row-cols-md-4 g-4">
          <div v-for="(art, index) in fetchDataList" :key="index">
            <AuctionArtSearchComponent v-if="$store.getters.isCurrentRequestEqualsAuctionArt === true" :art="art"/>
            <GeneralArtSearchComponent v-if="$store.getters.isCurrentRequestEqualsGeneralArt === true" :art="art"/>
          </div>
        </div>
      </div>
    </div>
    ...
    <ul class="pagination justify-content-center pagination-circle">
      <li v-if="pagination.prev === true" class="page-item">
        <b-button v-if="$store.getters.isCurrentRequestEqualsKeyword" class="page-link" @click="moveToPreviousPageWithKeywordSearch()">Previous</b-button>
        <b-button v-if="$store.getters.isCurrentRequestEqualsHashtag" class="page-link" @click="moveToPreviousPageWithHashtagSearch()">Previous</b-button>
      </li>
      <li v-for="(index) in range" :key="index" :class="checkActive(pagination.currentPage, index)" class="page-item">
        <b-button v-if="$store.getters.isCurrentRequestEqualsKeyword" class="page-link" @click="moveToIndexPageWithKeywordSearch(index)">{{ index }}</b-button>
        <b-button v-if="$store.getters.isCurrentRequestEqualsHashtag" class="page-link" @click="moveToIndexPageWithHashtagSearch(index)">{{ index }}</b-button>
      </li>
      <li v-if="pagination.next === true" class="page-item">
        <b-button v-if="$store.getters.isCurrentRequestEqualsKeyword" class="page-link" @click="moveToNextPageWithKeywordSearch()">Next</b-button>
        <b-button v-if="$store.getters.isCurrentRequestEqualsHashtag" class="page-link" @click="moveToNextPageWithHashtagSearch()">Next</b-button>
      </li>
    </ul>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async fetchData() {
      try {
        let response;
        if (this.$store.getters.isCurrentRequestEqualsKeyword) {
          response = await axios.get('/api/arts/keyword', {
            params: this.$store.getters.getKeywordSearch
          });
        } else if (this.$store.getters.isCurrentRequestEqualsHashtag) {
          response = await axios.get('/api/arts/hashtag', {
            params: this.$store.getters.getHashtagSearch
          });
        }
    ...
    applyNewFetchData() {
      if (this.$store.getters.isCurrentRequestEqualsKeyword === true) { // keyword search
        this.$store.commit('changeKeywordSearchSortType', this.sortType.defaultType)
        this.fetchData()
        this.$router.replace({
          path: '/search',
          query: {
        ...
        this.$store.commit('changeHashtagSearchSortType', this.sortType.defaultType)
        this.fetchData()
        this.$router.replace({
          path: '/search',
          query: {
    ...
    changeToAuctionDateDESC() {
      this.$store.commit('changeSortType', this.sortType.auction.dateDESC)
      this.sortType.defaultType = 'rdate'
      this.applyNewFetchData()
    },
    changeToAuctionDateASC() {
      this.$store.commit('changeSortType', this.sortType.auction.dateASC)
      this.sortType.defaultType = 'date'
      this.applyNewFetchData()
    },
    changeToAuctionPriceDESC() {
      this.$store.commit('changeSortType', this.sortType.auction.priceDESC)
      this.sortType.defaultType = 'rprice'
      this.applyNewFetchData()
    },
    changeToAuctionPriceASC() {
      this.$store.commit('changeSortType', this.sortType.auction.priceASC)
      this.sortType.defaultType = 'price'
      this.applyNewFetchData()
    },
    changeToAuctionLikeDESC() {
      this.$store.commit('changeSortType', this.sortType.auction.likeDESC)
      this.sortType.defaultType = 'rlike'
      this.applyNewFetchData()
    },
    changeToAuctionLikeASC() {
      this.$store.commit('changeSortType', this.sortType.auction.likeASC)
      this.sortType.defaultType = 'like'
      this.applyNewFetchData()
    },
    changeToAuctionCountDESC() {
      this.$store.commit('changeSortType', this.sortType.auction.countDESC)
      this.sortType.defaultType = 'rcount'
      this.applyNewFetchData()
    },
    changeToAuctionCountASC() {
      this.$store.commit('changeSortType', this.sortType.auction.countASC)
      this.sortType.defaultType = 'count'
      this.applyNewFetchData()
    },

    changeToGeneralDateDESC() {
      this.$store.commit('changeSortType', this.sortType.general.dateDESC)
      this.sortType.defaultType = 'rdate'
      this.applyNewFetchData()
    },
    changeToGeneralDateASC() {
      this.$store.commit('changeSortType', this.sortType.general.dateASC)
      this.sortType.defaultType = 'date'
      this.applyNewFetchData()
    },
    changeToGeneralPriceDESC() {
      this.$store.commit('changeSortType', this.sortType.general.priceDESC)
      this.sortType.defaultType = 'rprice'
      this.applyNewFetchData()
    },
    changeToGeneralPriceASC() {
      this.$store.commit('changeSortType', this.sortType.general.priceASC)
      this.sortType.defaultType = 'price'
      this.applyNewFetchData()
    },
    changeToGeneralLikeDESC() {
      this.$store.commit('changeSortType', this.sortType.general.likeDESC)
      this.sortType.defaultType = 'rlike'
      this.applyNewFetchData()
    },
    changeToGeneralLikeASC() {
      this.$store.commit('changeSortType', this.sortType.general.likeASC)
      this.sortType.defaultType = 'like'
      this.applyNewFetchData()
    },
    ...
    moveToPreviousPageWithKeywordSearch() {
      this.$store.commit('changePageToPreviousWithKeywordSearch')
      this.applyKeywordSearchPageMove()
    },
    moveToIndexPageWithKeywordSearch(index) {
      this.$store.commit('changePageToIndexWithKeywordSearch', index)
      this.applyKeywordSearchPageMove()
    },
    moveToNextPageWithKeywordSearch() {
      this.$store.commit('changePageToNextWithKeywordSearch')
      this.applyKeywordSearchPageMove()
    },
    moveToPreviousPageWithHashtagSearch() {
      this.$store.commit('changePageToPreviousWithHashtagSearch')
      this.applyHashtagSearchPageMove()
    },
    moveToIndexPageWithHashtagSearch(index) {
      this.$store.commit('changePageToIndexWithHashtagSearch', index)
      this.applyHashtagSearchPageMove()
    },
    moveToNextPageWithHashtagSearch() {
      this.$store.commit('changePageToNextWithHashtagSearch')
      this.applyHashtagSearchPageMove()
    }
  }
}
